...
if wlmData.dll.GetWLMCount(0) == 0:
    ...
    # Read frequency
    Frequency = wlmData.dll.GetFrequencyNum(8, 0.0)
    ...
    Exposure = wlmData.dll.GetExposureNum(8, 1, 0)
    ...
    # Read Ch8 Exposure mode
    ExpoMode = wlmData.dll.GetExposureModeNum(8, False)
    if ExpoMode == 1:
        print("Ch8 Exposure: Auto")
    ...
    if Frequency == wlmConst.ErrOutOfRange:
        print("Ch8 Error: Out of Range")
    elif Frequency <= 0:
        print("Ch8 Error code: %d" % Frequency)
    else:
        print("Ch8 Frequency: %f THz" % Frequency)